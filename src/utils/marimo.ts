...
export function getMarimoUrl(code: string): string {
  const compressed = compressToEncodedURIComponent(code);
  return `https://molab.marimo.io/new/wasm/?edit=true#code/${compressed}`;
}
...
def _(earthaccess):
  ...
  if (steps.length > 0) {
    ...
    steps.forEach((step, idx) => {
      mdIntro += `${idx + 1}. **${step.toolName}** - \`${JSON.stringify(step.params)}\`\\n`;
    });
...
def _(mo):
    ...
    return BBox, Client, Collection, Format, Request, client, downloaded_files, job_id, request`);
    }
...
def _(earthaccess, mo, plt, xr):
    mo.md("## ${isMap ? "Time-Averaged Spatial Map" : "Area-Averaged Time Series Plot"}")
    
    # Load dataset
    search_res = earthaccess.search_data(short_name="${coll}", count=5)
    files = earthaccess.open(search_res)
    ds = xr.open_mfdataset(files)
    
    var_key = "${varName}" if "${varName}" in ds else list(ds.data_vars.values())[0].name
    da = ds[var_key]
    ...
    map_data.plot(ax=ax, cmap="viridis")
    ax.set_title("${coll} - ${varName} Time-Averaged Map", fontweight="bold")`
        : `lat_dim = [d for d in da.dims if "lat" in d or "y" in d][0] if any("lat" in d or "y" in d for d in da.dims) else da.dims[-2]
    lon_dim = [d for d in da.dims if "lon" in d or "x" in d][0] if any("lon" in d or "x" in d for d in da.dims) else da.dims[-1]
    ts = da.mean(dim=[lat_dim, lon_dim])
    ts.plot(ax=ax, marker="o", color="#1e40af", linewidth=2)
    ax.set_title("${coll} - ${varName} Time Series", fontweight="bold")
    ax.grid(True, linestyle="--", alpha=0.6)`
    }
    plt.tight_layout()
    return ax, da, ds, fig, files, search_res`);
    }
  }
...
def _(earthaccess, mo, plt, xr):
    ...
    results = earthaccess.search_data(short_name="${coll}", count=5)
